...
export function Shell({ children }: { children: ReactNode }) {
  ...
  return <div className={isShowcase ? "app-shell showcase-shell" : "app-shell"}>
    <aside className={isShowcase ? "sidebar showcase-public-nav" : "sidebar"}>
      ...
      {isDemo ? <div className="sidebar-login demo-sidebar-login"><strong>Zero credentials</strong><span className="muted">This walkthrough never connects to Binance or an LLM.</span></div> : isShowcase ? <div className="sidebar-login demo-sidebar-login"><strong>Public read-only</strong><span className="muted">No owner session is required to inspect stored live evidence.</span></div> : <form action={login} className="sidebar-login"><label>Owner session<input name="password" type="password" required aria-label="Owner password" /></label><button className="button secondary" type="submit">Sign in</button>{authStatus && <p className="form-status" role="status">{authStatus}</p>}</form>}
