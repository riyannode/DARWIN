...
const modes = ["READ_ONLY", "APPROVAL_REQUIRED", "AUTO_BOUNDED"] as const;
...
export default function AgentPage() {
  ...
  return <div className="page"><div className="page-heading"><div><p className="eyebrow">AGENT / MANDATE</p><h1>Give it a mandate.</h1><p className="hero-copy">DARWIN monitors and decides continuously. Every BUY or SELL becomes a durable proposal requiring operator approval.</p></div><div className="agent-state"><span className="signal-dot" />{agent?.state ?? "Not connected"}</div></div><section className="two-column"><article className="panel"><div className="panel-heading"><div><p className="eyebrow">FOUR-PART MANDATE + EXECUTION POLICY</p><h2>What should it consider?</h2></div>{agent?.mandate !== null && agent?.mandate !== undefined && <span className="state-pill">Versioned</span>}</div><MandateForm /></article><article className="panel"><p className="eyebrow">OPERATING MODE</p><h2>How much autonomy?</h2><div className="mode-list">{modes.map((mode) => <button key={mode} className={agent?.mode === mode ? "mode-option selected" : "mode-option"} onClick={() => setMode(mode)}><span>{mode}</span><small>{mode === "AUTO_BOUNDED" ? "Monitor, decide, and signal; approval required for writes" : mode === "READ_ONLY" ? "Analyze without execution" : "Create intents for owner approval"}</small></button>)}</div><div className="button-row agent-actions"><button className="button primary" onClick={() => act("/api/agent/start")}>Start agent</button><button className="button secondary" onClick={() => act("/api/agent/run-once")}>Run once</button><button className="button secondary" onClick={() => act("/api/agent/stop")}>Stop agent</button></div>{agent?.nextRunAt && <p className="panel-note">Next scheduled cycle: {new Date(agent.nextRunAt).toLocaleString()}</p>}{status && <p className="form-status" role="status">{status}</p>}<div className="callout"><strong>Deterministic execution policy.</strong><p>Allowed symbols, maximum order notional, open-intent limit, budget, fresh account state, and exchange filters are backend authority. DARWIN cannot override them.</p></div></article></section></div>;
}
