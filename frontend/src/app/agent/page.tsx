...
export default function AgentPage() {
  ...
  return <div className="page"><div className="page-heading"><div><p className="eyebrow">AGENT / MANDATE</p><h1>Give it a mandate.</h1><p className="hero-copy">Strategy describes intent. The execution gateway decides whether a buy fits the rolling budget.</p></div><div className="agent-state"><span className="signal-dot" />{agent?.state ?? "Not connected"}</div></div><section className="two-column"><article className="panel"><div className="panel-heading"><div><p className="eyebrow">FOUR-PART MANDATE</p><h2>What should it consider?</h2></div>{agent?.mandate !== null && agent?.mandate !== undefined && <span className="state-pill">Versioned</span>}</div><MandateForm /></article><article className="panel"><p className="eyebrow">OPERATING MODE</p><h2>How much autonomy?</h2><div className="mode-list">{modes.map((mode) => <button key={mode} className={agent?.mode === mode ? "mode-option selected" : "mode-option"} onClick={() => setMode(mode)}><span>{mode}</span><small>{mode === "AUTO_BOUNDED" ? "Act without per-trade approval" : mode === "READ_ONLY" ? "Analyze without execution" : "Create intents for owner approval"}</small></button>)}</div><div className="button-row agent-actions"><button className="button primary" onClick={() => act("/api/agent/start")}>Start agent</button><button className="button secondary" onClick={() => act("/api/agent/run-once")}>Run once</button><button className="button secondary" onClick={() => act("/api/agent/stop")}>Stop agent</button></div>{agent?.nextRunAt && <p className="panel-note">Next scheduled cycle: {new Date(agent.nextRunAt).toLocaleString()}</p>}{status && <p className="form-status" role="status">{status}</p>}<div className="callout"><strong>One hard trading guard.</strong><p>Only the rolling 24-hour buy budget is enforced as a strategy-independent boundary. There is no per-order cap or pair allowlist.</p></div></article></section></div>;
}
